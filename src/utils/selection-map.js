export default function selectionMap({ series, films }) {
    return {
        ...
        films: [
            { title: 'Drama', data: films.filter((items) => items.genre === 'drama') },
            { title: 'Thriller', data: films.filter((items) => items.genre === 'thriller') },
            { title: 'Children', data: films.filter((items) => items.genre === 'children') },
            { title: 'Suspense', data: films.filter((items) => items.genre === 'suspense') },
            { title: 'Romance', data: films.filter((items) => items.genre === 'romance') }
        ]
    }
}